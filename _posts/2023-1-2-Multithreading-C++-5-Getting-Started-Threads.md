...
{:.filename}
```
Main thread has ID 6728  -> Diffirent thread
Hello thread has ID 8240 -> Same thread
Hello from thread with ID 8240 -> Same thread
Hello thread now has ID 0 -> Finish 
```
...
int main() {
	...
	std::thread thr(hello); //-> After 2s reveal "Hello, Thread!"
...
# II. The C++ Thread Class
- Implemented using RAII, RAII stands for "Resource Acquisition Is Initialization" and is a programming idiom used in several object-oriented, statically typed programming languages, including C++. The principle behind RAII is that an object's creation and destruction are tied to a resource being acquired and released. This means that when an object is created, it acquires the resource it needs, and when it is destroyed, it releases the resource it owns. RAII is used to manage resources such as memory, files, sockets, and lock. The constructor acquires a resource and the destructor releases the resource.
- An ```std::thread``` object has ownership of an execution thread, only one object can be bound to an execution thread at a time.
